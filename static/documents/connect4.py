...
def minimax(board, depth, alpha, beta, maximizingPlayer):
    ...
    if maximizingPlayer:
        ...
        for column in range(7):  # There are only 7 possible moves for the AI
            # Make a temporary board to use as reference
            temp_board = copy_board(board)
            row = openRow(temp_board, column)  # Get the next open row
            if row >= 0:
                # Making a move to the temporary board
                temp_board = MakeMove(temp_board, row, column, 'B')
                # Leveraging the benefits of recursion to implement the algorithm
                eval = minimax(temp_board, depth - 1, alpha, beta, False)[1]
                if eval > maxEval:
                    maxEval = eval
                    move = column
...
while not game_finished:
    ...
    if turn == 'R':
        ...
        column = int(
            input("Player, what column would you like to insert in? ")) - 1
        row = openRow(board, column)  # Getting the next available row

        # If the column is full prompt the user to insert another column
        while (row == -1):
            column = int(input(
                f"Column {column + 1} is taken. Please, Choose another column to play: ")) - 1
